...
class RandomBuffer(object):
	def __init__(self, state_dim, action_dim, Env_with_dead, max_size=int(1e6)):
		...
		# self.action_safe = np.zeros((max_size, num_RIS_elements))
		# self.pu = np.zeros((max_size, 1))
	...
	def sample(self, batch_size):
		...
		with torch.no_grad():
			return (
				...
				# torch.FloatTensor(self.action_safe[ind]).to(self.device),
				# torch.FloatTensor(self.pu[ind]).to(self.device),
